...
const ACCESS_KEYS = {
  ...
  storageAccessKey: getEnv("BUNNY_STOARGE_ACCESS_KEY"),
};
...
export const getVideoByIdDB = withErrorHandling(async (videoId: string) => {
...
export const getVideoByAnyId = withErrorHandling(async (videoId: string) => {
  // First try with DB id
  let record = await getVideoByIdDB(videoId);

  // If not found, try with videoId
  if (!record) {
    record = await getVideoById(videoId);
  }
